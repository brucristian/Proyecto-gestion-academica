...
public class ScheduleController {
    ...
    private void createColumns() {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            scheduleGrid.add(createHeader(days[i]), i + 1, 0);
        }
        scheduleGrid.add(createHeader("HORA"), 0, 0);
    }

    private void createRows(List<ScheduleItem> items) {
        scheduleGrid.getRowConstraints().clear();
        List<Integer> hours = items.stream()
                .flatMap(item -> item.hoursUsed().stream())
        ...
        for (Integer hour : hours) {
            ...
            StackPane hourPane = new StackPane(label);
            hourPane.setStyle("""
    ...
    private void addEvents(List<ScheduleItem> items) {
        List<Integer> visibleHours = items.stream()
                .flatMap(item -> item.hoursUsed().stream())
                .distinct().sorted().toList();

        for (ScheduleItem item : items) {
            try {
                FXMLLoader loader = new FXMLLoader(getClass().getResource("/ui/fxml/components/schedule_event/schedule_event.fxml"));
                Node node = loader.load();
                ScheduleEventController controller = loader.getController();
                controller.setData(item.courseName(), item.room(), item.start() + " - " + item.end());

                int column = item.day().columnIndex();
                int row = visibleHours.indexOf(item.start().getHour()) + 1;
                int span = item.end().getHour() - item.start().getHour();

                scheduleGrid.add(node, column, row);
